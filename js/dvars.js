 ...
 *
 * Descriptions come from the CSV export. Type, default value and range were
 * read out of the KisakCOD sources by parsing the Dvar_Register* calls,
 * including the ones that keep their bounds in a domain struct assigned just
 * above the call. 1163 of the 1196 were found there. FLT_MAX and INT_MAX are
 * reported as no limit rather than as a number, and bool keeps only its
 * default because true or false already states the domain.
 *
 * "|" needs no escaping: it occurs in none of the fields, and neither does a
 * backtick or "${".
 */
(function ()
{
...
bg_shock_screenType|Shell shock screen effect type|enum|0||
...
bg_shock_soundRoomType|Shell shock sound room type|enum|0||
...
cg_drawFPS|Draw frames per second|enum|1||
...
cg_drawMaterial|Draw debugging information for materials|enum|0||
...
cg_drawTalk|Controls which icons CG_TALKER ownerdraw draws|enum|1||
...
createserver_maps|||||
debug_protocol|||||
dedicated|Dedicated server|enum|2||
...
g_entinfo|Display entity information|enum|0||
...
r_aaAlpha|Transparency anti-aliasing method|enum|1||
...
r_clear|Controls how the color buffer is cleared|enum|1||
...
r_colorMap|Replace all color maps with pure black or pure white|enum|1||
...
r_debugShader|Enable shader debugging information|enum|0||
...
r_forceLod|Force all level of detail to this level|enum|4||
...
r_gpuSync|GPU synchronization type (used to improve mouse responsiveness)|enum|1||
...
r_lightMap|Replace all lightmaps with pure black or pure white|enum|1||
...
r_normalMap|Replace all normal maps with a flat normal map|enum|1||
...
r_rendererInUse|The renderer currently used|enum|2||
r_rendererPreference|Preferred renderer; unsupported renderers will never be used.|enum|2||
...
r_showFbColorDebug|Show front buffer color debugging information|enum|0||
...
r_showPixelCost|Shows how expensive it is to draw every pixel on the screen|enum|0||
...
r_specularMap|Replace all specular maps with pure black (off) or pure white (super shiny)|enum|1||
...
r_texFilterMipMode|Forces all mipmaps to use a particular blend between levels (or disables mipping.)|enum|0||
...
snd_draw3D|Draw the position and info of world sounds|enum|0||
snd_drawInfo|Draw debugging information for sounds|enum|0||
...
snd_outputConfiguration|Sound output configuration|enum|0||
...
sys_lockThreads|Prevents specified threads from changing CPUs; improves profiling and may fix some bugs, but can hurt performance|enum|0||
...
`;
	...
	function metaHtml(entry)
	{
		...
		return parts.length === 0 ? "" : '<p class="dv-meta">' + parts.join(" &middot; ") + "</p>";
	}
